...
class crch():
    ...
    Input Variables of the MODFLOW CFP CRCH Module:
        ...
        # p_crch : is a list-like of floats of length n_tubes representing the fraction of diffuse areal recharge (entered in the MODFLOW-2005 RCH Package) partitioned directly into the conduit node NODE_NUMBERS.
            If the user, for example, wants the direct conduit recharge to equal the diffuse recharge rate assigned for the model cell in which the pipe is located, the user would enter a value of 1.0 for P_CRCH. 
            In this case, the diffuse areal recharge for the model cell would equal zero in MODFLOW water-budget calculations
        # fbc_well : is a list with tuples containing node number and specified pumping
            rate during all / different stress periods [(node_num, rate), (...), ...], if
            the pumping rate is uniform over all stress periods, use a list with a single
            float value as secont tuple item and if the pumping rate is non-uniform,
            use a list of length nper; list of tuples
            example: [(1, [-0.15]), (5, [-0.1])] or [(1, [-0.1, -0.3])]
